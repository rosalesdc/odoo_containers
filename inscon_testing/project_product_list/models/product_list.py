	...
	def purchase_order(self,seller,producto,currency_id):
		...
		for products in producto:
			...
			vals_order_line = {
				...
				'name':product.name,
				'account_analytic_id':proyecto_id.id,
...
class productListLine(models.Model):
	...
	def get_proveedor(self):
		string = ""
		for record in self:
			partner = record.supplier_id.name if not record.supplier_id.name.parent_id else record.supplier_id.name.parent_id
			if not record.product.seller_ids.mapped('name') and len(record.product.seller_ids) <= 10:
				string = string + record.product.name + "\n"
				raise ValidationError(_('El Producto %s no tiene asignado proveedor.'%string))
